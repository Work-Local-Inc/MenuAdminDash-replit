...
async function registerDomains() {
  ...
  const domains = [
    'orders.menu.ca',
    'menu.ca',
    'seasonspizzaottawa.menu.ca',
    'centertowndonair.menu.ca', 
    'orchidsushiottawa.menu.ca',
    'goldencenterpizza.menu.ca',
    'pizzaliciousottawa.menu.ca',
  ];
  ...
  try {
    ...
    for (const d of pmDomains.data) {
      console.log(`  ${d.domain_name}: apple_pay=${d.apple_pay?.status}, google_pay=${d.google_pay?.status}`);
    }
    if (pmDomains.data.length === 0) {
      ...
      for (const domain of domains) {
        try {
          ...
          console.log(`  ✓ Created payment method domain: ${domain} - apple_pay=${result.apple_pay?.status}, google_pay=${result.google_pay?.status}`);
        } catch (err: any) {
