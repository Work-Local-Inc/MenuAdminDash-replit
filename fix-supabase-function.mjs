...
console.log('🔧 Fixing get_restaurant_menu SQL function...\n');
...
BEGIN
  ...
  active_dishes AS (
    SELECT
      ...
      d.image_url, d.is_active, d.is_featured, d.has_customization,
          ...
          'modifiers', (
            SELECT jsonb_agg(
              jsonb_build_object(
                ...
                'name', COALESCE(dmt.name, dm.name), 'price', dm.price,
                'is_included', dm.is_included, 'is_default', dm.is_default,
                'display_order', dm.display_order, 'created_at', dm.created_at,
  ...
  dishes_with_details AS (
    SELECT
      ...
      jsonb_build_object(
        ...
        'is_active', ad.is_active, 'is_featured', ad.is_featured,
...
const { error: createError } = await supabase.rpc('exec_sql', { sql: functionSQL }).catch(async () => {
  // If exec_sql doesn't exist, try direct query
  return await supabase.from('_sql').insert({ query: functionSQL }).select().single();
}).catch(async () => {
  // Last resort: use Postgres connection
  console.log('📝 Attempting to execute SQL via direct connection...');
  const { createClient: createPgClient } = await import('@supabase/supabase-js');
  const adminClient = createPgClient(supabaseUrl, supabaseKey);
  return await adminClient.rpc('exec', { sql: functionSQL });
});
...
// Test the function with restaurant 961
...
const courseCount = testData?.courses?.length || 0;
const totalDishes = testData?.courses?.reduce((sum, c) => sum + (c.dishes?.length || 0), 0) || 0;

console.log('✅ Test Results:');
console.log(`   Restaurant ID: ${testData?.restaurant_id}`);
...
console.log('\n✅ Fix complete! Frontend should now work.');
