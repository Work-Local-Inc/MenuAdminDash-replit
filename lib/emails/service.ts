...
async function sendEmailWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt > 1) {
        console.log(
          `[Email Service] ${context} succeeded on attempt ${attempt}/${maxRetries}`,
        );
      }
      ...
      console.error(
        `[Email Service] ${context} failed (attempt ${attempt}/${maxRetries}):`,
        {
...
function generateOrderConfirmationPlainText(
  ...
  if (data.taxBreakdown && data.taxBreakdown.length > 0) {
    data.taxBreakdown.forEach((taxItem) => {
      const rateStr =
        taxItem.rate === 0.09975
          ? "9.975%"
          : `${(taxItem.rate * 100).toFixed(0)}%`;
  ...
  if (isPickup && data.pickupLocation) {
    ...
    lines.push(
      `${data.pickupLocation.city}, ${data.pickupLocation.province} ${data.pickupLocation.postal_code}`,
    );
    ...
    lines.push(
      `${data.deliveryAddress.city}, ${data.deliveryAddress.province} ${data.deliveryAddress.postal_code}`,
    );
...
function generateWelcomeEmailPlainText(data: WelcomeEmailData): string {
  ...
  lines.push("");
  lines.push(
    "Visit: " + (process.env.NEXT_PUBLIC_APP_URL || "https://menuai.ca"),
  );
  lines.push("");
  lines.push("========================================");
  lines.push("");
...
export async function sendOrderConfirmationEmail(
  ...
  try {
    if (!process.env.RESEND_API_KEY) {
      console.warn(
        "[Email Service] RESEND_API_KEY not configured - skipping order confirmation email",
        {
...
export async function sendWelcomeEmail(data: WelcomeEmailData): Promise<void> {
  ...
  try {
    if (!process.env.RESEND_API_KEY) {
      console.warn(
        "[Email Service] RESEND_API_KEY not configured - skipping welcome email",
        {
          email: data.email,
        },
      );
