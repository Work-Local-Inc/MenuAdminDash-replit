...
export async function getRestaurants(filters?: {
  ...
  try {
    let query = supabase
      .from("restaurants")
      .select(
        "*, restaurant_locations!restaurant_locations_restaurant_id_fkey(city_id, province_id, is_primary, cities(name))",
      )
    ...
    if (
      ...
      filters.allowedRestaurantIds.length > 0
    ) {
    ...
    console.log(
      `[getRestaurants] Found ${data?.length || 0} restaurants with filters:`,
      filters,
    );

    return (data || []).map((restaurant: any) => {
      const primaryLocation = restaurant.restaurant_locations?.find(
...
export async function getRestaurantById(id: string) {
  const supabase = createAdminClient() as any;
  const { resolveIdParam } = await import("@/lib/utils/uuid");
  const { column, value } = resolveIdParam(id);
...
export async function getOrders(filters?: {
  ...
  try {
    ...
    const buildQuery = (includeGuestColumns: boolean) => {
      const selectClause = includeGuestColumns
        ? guestColumns + baseColumns
        : baseColumns;
      let query = supabase
        .from("orders")
        .select(selectClause)
    ...
    if (error) {
      console.log(
        "[getOrders] Guest columns query failed, trying without guest columns:",
        error.message,
      );
...
export async function getDashboardStats(allowedRestaurantIds?: number[]) {
  ...
  try {
    ...
    let ordersQuery = supabase
      ...
      .select("total_amount")
      .or("is_test_order.is.null,is_test_order.eq.false");
    let restaurantsCountQuery = supabase
      .from("restaurants")
      .select("*", { count: "exact", head: true })
      .eq("status", "active");
    ...
    if (allowedRestaurantIds && allowedRestaurantIds.length > 0) {
      ordersCountQuery = ordersCountQuery.in(
        "restaurant_id",
        allowedRestaurantIds,
      );
      ordersQuery = ordersQuery.in("restaurant_id", allowedRestaurantIds);
      restaurantsCountQuery = restaurantsCountQuery.in(
        "id",
        allowedRestaurantIds,
      );
      recentOrdersQuery = recentOrdersQuery.in(
        "restaurant_id",
        allowedRestaurantIds,
      );
    }
    ...
    const restaurantIds = new Set(
      (recentOrdersRes.data || []).map((o: any) => o.restaurant_id),
    );
