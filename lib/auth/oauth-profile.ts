...
export async function ensureOAuthProfileForSession(
  ...
  const { authUserId, email, emailVerified, firstName, lastName, phone } =
    input;
  ...
  try {
    ...
    if (existingUserByAuth) {
      ...
      console.log(
        "[OAuth Profile] User already exists:",
        existingUserByAuth.id,
      );
    ...
    const { data: usersWithEmail } = (await supabase
      .from("users")
      .select(
        "id, auth_user_id, stripe_customer_id, first_name, last_name, phone",
      )
      .eq("email", email)) as {
      data:
        | {
    ...
    if (usersWithEmail && usersWithEmail.length > 0) {
      ...
      if (hasConflict) {
        console.error(
          "[OAuth Profile] Email already associated with different authenticated account",
        );
        return {
          success: false,
      ...
      if (guestAccount) {
        ...
        console.log(
          "[OAuth Profile] Upgrading guest account:",
          guestAccount.id,
        );
        ...
        const { data: updatedUser, error: updateError } = (await (
          ...
          .single()) as {
          data: { id: number; stripe_customer_id: string | null } | null;
          error: any;
        };

        if (updateError || !updatedUser) {
          console.error(
            "[OAuth Profile] Error upgrading guest account:",
            updateError,
          );
...
async function ensureStripeCustomer(
  ...
  try {
    ...
    console.error(
      "[OAuth Profile] Error creating Stripe customer (non-blocking):",
      stripeError,
    );
