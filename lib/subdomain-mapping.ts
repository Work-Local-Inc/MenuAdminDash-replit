...
async function fetchMappingsFromDatabase(): Promise<SubdomainMapping[] | null> {
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.log(
        "[Subdomain] Missing Supabase credentials, using static fallback",
      );
      return null;
    }
    ...
    const response = await fetch(
      `${supabaseUrl}/rest/v1/rpc/get_all_subdomain_mappings`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        console.log(
          "[Subdomain] RPC function not found, using static fallback",
        );
        return null;
      }
    ...
    console.log(
      `[Subdomain] Loaded ${mappings.length} mappings from database:`,
      mappings.map((m) => m.subdomain).join(", "),
    );
    return mappings;
  } catch (error) {
...
export async function getAllMappings(): Promise<SubdomainMapping[]> {
  ...
  const dbSubdomains = new Set(
    (dbMappings || []).map((m) => m.subdomain.toLowerCase()),
  );
...
export function isDevHostname(hostname: string): boolean {
  ...
  return DEV_HOSTNAMES.some(
    (dev) => lowerHost === dev || lowerHost.endsWith(dev),
  );
}
 ...
 * Example: orchidsushiottawa.menuai.ca → orchidsushiottawa
...
export function extractSubdomain(hostname: string): string | null {
  ...
  // Check for .menuai.ca or .menu.ca suffix
  let subdomain: string | null = null;
  if (lowerHost.endsWith(".menuai.ca")) {
    subdomain = lowerHost.replace(".menuai.ca", "");
  } else if (lowerHost.endsWith(".menu.ca")) {
    subdomain = lowerHost.replace(".menu.ca", "");
  }
 ...
 */
export function getRestaurantBySubdomain(
  subdomain: string,
): SubdomainMapping | null {
  const mappings = getAllMappingsSync();
  const lowerSubdomain = subdomain.toLowerCase();
  return (
    mappings.find((m) => m.subdomain.toLowerCase() === lowerSubdomain) || null
  );
}
 ...
 */
export async function getRestaurantBySubdomainAsync(
  subdomain: string,
