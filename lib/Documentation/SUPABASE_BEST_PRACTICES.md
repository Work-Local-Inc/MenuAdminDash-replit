...
**Applies To:** Menu.ca v3 Supabase PostgreSQL Implementation
...
*   **Use Zod for validation:** Implement comprehensive Zod schemas for all API routes (see `lib/validation/promotions.ts` for example).
...
*   **Restaurant-level permissions:** For multi-tenant systems, always filter by authorized restaurant IDs (see `lib/api/promotions.ts`).
...
*   **Never allow privilege escalation:** Use strict Zod schemas and SQL filters to prevent cross-resource updates (see promotional deals PATCH route for example).
...
*   **Use `run_test` tool:** For Menu.ca, use the built-in Playwright testing subagent to validate features.
...
*   **Forgetting schema configuration:** Not including `db: { schema: 'menuca_v3' }` in Supabase client initialization.
*   **Mixing ID types:** Confusing integer IDs with UUIDs (Menu.ca uses INTEGER restaurant IDs).
...
*   **Null handling:** Always use safe parsing for numeric aggregates from Supabase (see analytics page for example).
...
*   **Check workflow logs:** Use `refresh_all_logs` tool to examine server errors.
...
*   **Drizzle ORM:** For database schema management (if using ORM).
*   **Postman/Insomnia:** For testing API endpoints.

### 7.2. Build Configuration

*   Use a build tool like Webpack or Vite to bundle your code.
...
*   **Use Replit deployments:** Publish via Replit's built-in deployment system.
...
## 10. Menu.ca-Specific Guidelines

### 10.1. Schema Configuration (CRITICAL)

**ALL Supabase clients MUST include `db: { schema: 'menuca_v3' }`**
...
See **[SUPABASE_CONFIG.md](../../SUPABASE_CONFIG.md)** for complete configuration details.

### 10.2. Restaurant Permission Pattern

For all restaurant-specific operations:
...
// 2. Verify permission for specific restaurant
...
**Key Resources:**
- **[SUPABASE_CONFIG.md](../../SUPABASE_CONFIG.md)** - Critical schema configuration
- **[API Architecture Guide](./API_ARCHITECTURE_GUIDE.md)** - Complete API patterns and examples
- **[Supabase Official Docs](https://supabase.com/docs)** - Latest features and updates
