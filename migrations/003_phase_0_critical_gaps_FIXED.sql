-- ============================================
-- Menu.ca V3 - Phase 0 Critical Gaps (FIXED)
...
-- ============================================
-- GAP #3: SERVER-SIDE PRICE VALIDATION (FIXED)
...
BEGIN
  ...
  LOOP
    ...
    -- Calculate modifier prices (FIXED: proper accumulation)
    ...
    IF v_item->'modifier_ids' IS NOT NULL THEN
      ...
      LOOP
        ...
        -- FIXED: Accumulate modifier prices correctly
  ...
  -- Get delivery fee if delivery (from restaurant record)
...
COMMENT ON FUNCTION menuca_v3.calculate_order_total IS 'Server-side order total calculation with availability checks - CRITICAL SECURITY: Never trust client prices!';

-- ============================================
-- GAP #4: CART AVAILABILITY CHECK (FIXED)
-- Check if all items in cart are still available
-- Uses dish_inventory table correctly
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    -- FIXED: Check dish_inventory table for availability
    SELECT * INTO v_inventory
    FROM menuca_v3.dish_inventory
    WHERE dish_id = (v_item->>'dish_id')::BIGINT;

    -- If inventory record exists, check availability
    IF v_inventory.dish_id IS NOT NULL THEN
      ...
      END IF;
    END IF;
    -- If no inventory record, dish is assumed available
  END LOOP;
...
COMMENT ON FUNCTION menuca_v3.check_cart_availability IS 'Validates all cart items are available before checkout using dish_inventory table';

-- ============================================
-- GAP #5: ORDER CANCELLATION FUNCTION (FIXED)
-- Added ownership/auth checks
-- ============================================
...
BEGIN
  ...
  IF v_order.is_guest_order = TRUE THEN
    ...
    END IF;
  ELSE
    -- User order: verify user_id matches
...
COMMENT ON FUNCTION menuca_v3.can_cancel_order IS 'Checks if order can be cancelled with ownership verification';

-- ============================================
-- VERIFICATION
-- ============================================

SELECT 'Phase 0 critical gaps migration completed (FIXED VERSION)!' AS status,
       'Created: dish_inventory table, 4 SQL functions with security fixes' AS created,
       'Updated: orders table (guest checkout fields, nullable user_id)' AS updated,
       'Security: Server-side price calculation (FIXED), availability checks, ownership validation' AS security_improvements;
