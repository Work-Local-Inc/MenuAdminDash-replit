...
export async function GET(
  ...
  try {
    ...
    const timezone =
      (restaurant as any)?.restaurant_locations?.[0]?.cities?.timezone ||
      "America/Toronto";

    const fourWeeksAgo = new Date();
    fourWeeksAgo.setDate(fourWeeksAgo.getDate() - WEEKS_TO_ANALYZE * 7);

    const ordersFk = resolveFkParam(
      params.id,
      "restaurant_id",
      "restaurant_uuid",
    );
    const { data: orders, error } = await supabase
      .from("orders")
      .select("created_at")
      .eq(ordersFk.column, ordersFk.value)
    ...
    if (!orders || orders.length < 20) {
      return NextResponse.json({
        ...
        message:
          "Not enough order history to detect peak hours (minimum 20 orders needed)",
        hourly_data: [],
        timezone,
      });
    }
    ...
    for (const candidate of sortedCandidates) {
      if (!currentPeak) {
        currentPeak = {
          day: candidate.day,
          start: candidate.hour,
          end: candidate.hour + 1,
        };
      } else if (
        currentPeak.day === candidate.day &&
        currentPeak.end === candidate.hour
      ) {
...
export async function POST(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .update(updateData)
      .eq(configFk.column, configFk.value)
      .select()
      .single();
