...
const DASHBOARD_URL =
  process.env.ONBOARDING_DASHBOARD_URL || "https://menuai.ca/login";
...
function generatePlainText(
  ...
  lines.push("");
  lines.push(
    `This is ${data.employeeName} from Worklocal. Thank you again for having me at your restaurant—the feedback you provided was very useful.`,
  );
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    "1. After logging in, click on the small gray circle with your initials in the top right corner to display the user options.",
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `If you have any questions or encounter any issues, please feel free to contact me${data.employeeContact ? ` via email or phone at ${data.employeeContact}` : ""}.`,
  );
  lines.push("");
  ...
  try {
    ...
    if (!body.adminEmail || !body.employeeName || !body.restaurantName) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: adminEmail, employeeName, restaurantName",
        },
        { status: 400 },
      );
    }
