...
import {
  getDeliveryProviderConfig,
  getDeliveryProviderAdapter,
...
export async function POST(
  ...
  try {
    ...
    const providerConfig = await getDeliveryProviderConfig(
      deviceContext.restaurant_id,
    );
    ...
    if (!postalCode && order.delivery_address) {
      const postalMatch = order.delivery_address.match(
    ...
    if (isDryRun) {
      console.log(`[Tablet Dispatch Driver] DRY RUN - Order ${orderId}:`, {
    ...
    console.log(
      `[Tablet Dispatch Driver] Order ${orderId} via ${providerConfig.provider.name}: ${dispatchResult.success ? "Success" : "Failed"}`,
