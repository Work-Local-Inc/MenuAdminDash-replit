...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: byAuthId } = await adminSupabase
      .from('users')
      .select('id, email, phone, first_name, last_name, auth_user_id, stripe_customer_id')
    ...
    if (existingUser) {
      ...
      if (!existingUser.first_name && first_name) updateData.first_name = first_name
      if (!existingUser.last_name && last_name) updateData.last_name = last_name
      ...
      if (Object.keys(updateData).length > 0) {
        ...
        console.log('[Ensure Profile] Updated existing user:', existingUser.id, Object.keys(updateData))
      }

      return NextResponse.json({ userId: existingUser.id, action: 'linked' }, { status: 200 })
    }
    ...
    if (stripe && (email || phone)) {
      try {
        ...
        console.error('[Ensure Profile] Stripe customer creation failed (non-blocking):', stripeErr.message)
      }
    }

    return NextResponse.json({ userId: newUser.id, action: 'created' }, { status: 201 })
  } catch (error: any) {
