...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[Forgot Password] Received body - email:",
      email,
      "returnTo:",
      returnTo,
      "redirectUrl:",
    ...
    console.log(
      "[Forgot Password] Password reset requested for:",
      normalizedEmail,
    );
    ...
    if (
      ...
      (linkError.message?.includes("not found") ||
        ...
        linkError.message?.includes("Unable to find"))
    ) {
      console.log(
        "[Forgot Password] User not found in Supabase Auth, auto-creating account for:",
        normalizedEmail,
      );
      ...
      if (createError) {
        console.error(
          "[Forgot Password] Failed to auto-create user:",
          createError.message,
        );
        return NextResponse.json(
    ...
    if (linkError) {
      console.error(
        "[Forgot Password] Error generating reset link:",
        linkError.message,
      );

      if (
        linkError.message?.includes("rate") ||
        linkError.message?.includes("Rate")
      ) {
        return NextResponse.json(
    ...
    if (redirectUrl && hashedToken) {
      const parsedOrigin = redirectUrl.startsWith("http")
      ...
      const baseOrigin = isAllowed ? parsedOrigin : "https://menuai.ca";
    ...
    const rawEmail = FROM_EMAIL.includes("<")
    ...
    console.log(
      "[Forgot Password] Sending email via Resend to:",
      normalizedEmail,
      "from:",
      fromAddress,
    );
    ...
    if (emailError) {
      console.error(
        "[Forgot Password] Error sending email via Resend:",
        emailError,
      );
      return NextResponse.json(
