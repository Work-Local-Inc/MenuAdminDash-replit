  ...
  try {
    ...
    if (!hasAccess) {
      ...
      return NextResponse.json(
        { error: "Access denied. Please check your link." },
        { status: 403 },
      );
    }
    ...
    const loc =
      order.restaurant?.restaurant_locations?.find(
        (l: any) => l.is_primary && l.is_active,
      ) ||
    ...
    const parsedItems = items.map((item: any) => ({
      ...
      modifiers:
        item.modifiers?.map((mod: any) => ({
          ...mod,
          price:
            typeof mod.price === "string" ? parseFloat(mod.price) : mod.price,
        })) || [],
    }));
