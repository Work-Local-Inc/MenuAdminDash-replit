...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract order type, service time, and order notes from payment intent metadata
    ...
    const parsedTip = parseFloat(metadata.tip_amount || '0')
    const tipAmount = Number.isFinite(parsedTip) ? parsedTip : 0
    ...
    console.log('[Order API] Order notes:', orderNotes)
    ...
    // which maps dish sizes to the correct modifier price tiers.
    // The dish_prices TABLE does not have this column, so we must use menu data.
      ...
      course.dishes?.forEach((dish: any) => {
        ...
        if (dish.prices && Array.isArray(dish.prices)) {
          dish.prices.forEach((p: any, idx: number) => {
            ...
            const existing = dishPriceOptions.get(dish.id) || []
            existing.push({ size_variant: p.size_variant, price: String(p.price) })
            dishPriceOptions.set(dish.id, existing)
          })
        }
      })
    })
    ...
    if (modifierIds.length > 0) {
      ...
      if (simpleModIds.length > 0) {
        ...
        simpleModifierPricesData?.forEach((priceRow: any) => {
          ...
          console.log(`[Order API] Modifier price: mod=${priceRow.modifier_id} size_variant_id=${sizeVariantId} price=${priceRow.price}`)
        })
      }
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const mod of item.modifiers) {
          ...
          if (simpleModifier) {
            ...
            // Use price from combo modifier with size variant lookup
            // Combo modifiers use size_variant string (e.g., "Large") rather than modifier_size_variant_id
            const sizeForCombo = item.size || 'base'
            let modPrice = comboModifierPriceMap.get(`${mod.id}-${sizeForCombo}`)
            // Fallback to base price if size-specific price not found
            if (modPrice === undefined) {
              modPrice = comboModifierPriceMap.get(`${mod.id}-base`) ?? 0
            }
    ...
    const serverTotal = serverSubtotal + deliveryFee + tax + tipAmount
    ...
    // Build special_instructions: combine order notes with scheduled time info
    const specialInstructions = (() => {
      const parts: string[] = []
      if (orderNotes?.trim()) parts.push(orderNotes.trim())
      if (serviceTime.type === 'scheduled' && serviceTime.scheduledTime) {
        parts.push(`Scheduled for: ${serviceTime.scheduledTime}`)
      }
    ...
    if (promoId && discountAmount > 0 && promoType === 'coupon') {
      try {
        ...
        console.log('[Order API] Coupon usage logged:', { promoId, discountAmount, orderId: order.id })
      } catch (couponLogError) {
        console.warn('[Order API] Failed to log coupon usage:', couponLogError)
      }
    }
